...
dat_1 <- rtry::rtry_import("~/Documents/TRY_leaf_trait_data/37407.txt")
dat_2 <- rtry::rtry_import("~/Documents/TRY_leaf_trait_data/38952.txt")
dat_3 <- rtry::rtry_import("~/Documents/TRY_leaf_trait_data/43511.txt")
...
write.csv(t(IDs), "data/TRY/Trait_IDs_wanted.csv", row.names = FALSE)
...
readLines("data/TRY/TryAccSpecies.txt")
species <- read.table("data/TRY/TryAccSpecies.txt", sep = "\t", 
...
# some of these values are on different scales we need t be aware of that
# look at the units
dat %>% 
  filter(SpeciesName %in% species_wanted) %>%
  filter(TraitName %in% traits) %>%
...
small <- 
  ...
  filter(SpeciesName %in% species_wanted) %>%
  filter(TraitName %in% traits)
write_csv(small, "results/my_species_TRY_data.csv")
...
write_csv(selected_sets, "results/selected_European_data_TRY_our_species.csv")
